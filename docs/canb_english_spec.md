...
```
1. 학생이 활동을 시작 → Full Round 1을 시작
   - 활동에 속한 모든 문항을 출제
   - Full Round가 시작되는 시점에 전체 문항 순서를 무작위 셔플
   - 각 문항을 순서대로 풀이
   - 매 문항 정답 제출 시 즉시 피드백 (정답/오답 + 해설)
2. Full Round 종료 시 점수 계산 = (정답 수 / 전체 문항 수) × 100
3. Full Round 점수가 100점이면 활동 완료, 결과 화면으로 이동
4. Full Round 점수가 100점 미만이면:
   - 틀린 문항만 모아 Review Round 출제 목록 생성
   - Review Round가 시작되는 시점에 오답 문항 순서를 무작위 셔플
   - Review Round 시작
5. Review Round 진행:
   - 직전 Full Round에서 틀린 문항만 다시 출제
   - 이 단계는 오답 복습용이며 점수 산정에 포함하지 않음
   - Review Round에서 모든 문항을 맞혀도 활동 완료 처리하지 않음
6. Review Round가 끝나면 Full Retry Round 시작
   - 다시 전체 문항을 모두 출제
   - Full Retry Round가 시작되는 시점에 전체 문항 순서를 다시 무작위 셔플
   - 전체 문항 기준으로 점수를 다시 계산
7. 전체 문항 기준 100점이 나올 때까지 4~6을 반복
8. "첫 시도 정답률"은 Full Round 1의 점수
9. "최고 점수"는 Full Round 점수들 중 가장 높은 점수
10. "최종 점수"는 완료 시 마지막 Full Round 점수이며, 완료 조건상 100점
11. Review Round 점수는 bestScore/finalScore에 반영하지 않음
```

**중요**:
- 활동 완료 조건은 오직 전체 문항 기준 100점이다.
- 틀린 문제만 다시 푸는 단계는 오답 복습용이며, 오답 복습만으로 활동 완료 처리하지 않는다.
- 오답 복습 후에는 반드시 전체 문항을 다시 풀어야 한다.
- Full Round, Full Retry Round는 전체 문항을 라운드 시작 시점에 셔플한다.
- Review Round는 직전 Full Round의 오답 문항만 라운드 시작 시점에 셔플한다.
- 객관식(`multiple_choice`)과 이항대립(`binary_choice`) 선택지는 화면 표시 시 무작위 셔플한다.
- 선택지를 셔플할 때는 원래 보기 인덱스(`originalIndex`)를 보존하고, 정답 판정은 `originalIndex === answer` 기준으로 한다.
- 같은 문제를 보고 있는 동안에는 선택지 순서가 고정되어야 하며, 답 선택/제출/피드백 표시로 다시 섞이면 안 된다.
- 같은 문항이 다른 라운드에서 다시 출제될 때는 선택지를 다시 셔플할 수 있다.
